...
describe("Straight Test - Netflix", () => {
  it("Selecting a Netflix", async () => {
    ...
    const paswResetBtn = $(
      "#appMountPoint > div > div.login-body > div > div > div.hybrid-login-form-main > div > div.ui-message-contents > a"
    );
    const forgotPaswInput = $("#forgot_password_input");
    const mailMe = $(
      "#appMountPoint > div > div.login-body > div.login-content > div > button"
    );
    // Netflix Sign in
    ...
    //Enter valid email below
    await enterMail.setValue("helhhhllllo92@mailsac.com");
    await browser.pause(1000);

    await enterPassword.setValue("LabaDieasna123");
    await browser.pause(1000);
    ...
    await signIn.click();
    await browser.pause(1000);
    await paswResetBtn.click();
    await forgotPaswInput.setValue("helhhhllllo92@mailsac.com");
    await mailMe.click();
